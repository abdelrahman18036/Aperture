...
- **The Quiet Gallery is the approved world.** Cool-lilac canvas, white/deep-navy cards, charcoal/light ink, softened violet accents, and generous rounded geometry come from the user-supplied reference.
- **Media remains the hero.** The 840px content lane and 20px media frames give photography the largest uninterrupted surface without hiding its social context.
- **Familiar interaction wins.** Use recognizable social icons, ordinary sentence case, 44px targets, visible focus, and explicit system states. Do not revive industrial labels, keycaps, amber strips, or orange commit controls.
- **One signature motion — the develop-in.** No scroll reveals in the feed, staggered grids, parallax, or particle effects. Other motion is functional, short, and reduced-motion safe.
- **Both themes are first-class.** Dark mode translates the same hierarchy into deep navy rather than inverting the light palette.

`DESIGN.md` is the full portable specification and `packages/ui/src/theme.css` is the implemented token authority. Design skills may be used when they support the current user-approved direction; they must not replace it with a different aesthetic.
