...
This applies to every clause that accepts a callback — `orderBy`, `where`.`RAW`, `extras` and `subqueries inside extras`. 
...
<Callout type="error" title='❌' collapsed="Example">
```ts
...
<Callout type="default" title='✅' collapsed="Example">
```ts
await db.query.posts.findMany({
